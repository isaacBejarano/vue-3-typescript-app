<template>
	...
	<form @submit.prevent="validateForm" class="form mxw-568 mx-auto text-left p-05 radius-sm shadow">
		...
		<label for="label2" class="d-block m-1">
			{{ labels[2] }}
			<select v-model="formdata.role" name="role" id="label2" class="ml-1" ref="2" @change="validateOnTouched('2')">
		...
		<label for="label3" class="d-block m-1">
			{{ labels[3] }}
			<input
				type="checkbox"
				name="lgpd"
